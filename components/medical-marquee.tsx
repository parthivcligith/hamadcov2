...
export default function MedicalMarquee() {
  ...
  return (
    <div className="relative overflow-hidden bg-gradient-to-r from-blue-50 via-white to-blue-50 py-6 md:py-8 border-y border-blue-100">
      <div className="absolute left-0 top-0 z-10 h-full w-16 md:w-32 bg-gradient-to-r from-blue-50 to-transparent" />
      <div className="absolute right-0 top-0 z-10 h-full w-16 md:w-32 bg-gradient-to-l from-blue-50 to-transparent" />
      ...
      >
        ...
        {medicalClients.map((client, index) => (
          <div
            key={`first-${index}`}
            className="flex items-center gap-3 md:gap-6 px-4 md:px-6 py-2 md:py-3 bg-white/70 backdrop-blur-sm rounded-full border border-blue-200/50 shadow-sm hover:shadow-lg hover:bg-blue-50/80 transition-all duration-300 group min-w-fit"
          >
            <div className="flex items-center gap-2 md:gap-3">
              <div className="w-1.5 h-1.5 md:w-2 md:h-2 bg-blue-500 rounded-full animate-pulse" />
              <div className="text-xs md:text-sm font-medium text-gray-900 group-hover:text-blue-600 transition-colors">
                {client.name}
              </div>
            </div>
            <div className="h-3 md:h-4 w-px bg-blue-300" />
            <div className="text-xs font-medium text-gray-600">{client.location}</div>
            <div className="hidden sm:block text-xs text-blue-600 bg-blue-100 px-2 py-1 rounded-md">
              {client.specialty}
            </div>
